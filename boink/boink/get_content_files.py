...
from boink.boink.config import *
...
def get_content_files_to_be_published(paths: list[Path]) -> list[dict]:
    ...
    and converts them to a list of file names, directories and configurations.
    ...
    ```
    {
        ...
        "config_dirs": [ "<directory for config assets>", ... ]
    }
    ```

    Throws `SeekingContentFiles` if something goes wrong.
    """
...
def _split_to_dir_and_filename(path: Path) -> tuple[DirName, FileName]:
    ...
    Throws `SeekingContentFiles` if file is not a markdown file,
    or dir is not found.
    """
    ...
    if os.path.isfile(path):
        ...
        raise SeekingContentFiles(f"{path} is not a file or directory")

    return (content_dir, content_file)


def _seek_content_root(content_dir: DirName) -> tuple[DirName, dict]:
    ...
    Throws `SeekingContentFiles` if no parent directory is found.
    """
    current_dir = os.path.abspath(content_dir)

    for limit in range(999):  # Better than `while True:`...
        for file in os.listdir(current_dir):
            if ROOT_CONFIG_PATH_RE.search(file):
                root_config_path = os.path.join(current_dir, file)
                with open(root_config_path) as stream:
                    root_config = yaml.safe_load(stream)
                    if root_config is None:
                        root_config = {}
                    if type(root_config) is not dict:
                        raise SeekingContentFiles(
                            f"{root_config_path} is not a YAML map",
    ...
    raise SeekingContentFiles(
        f".boink file or directory was not found in any of the parent directories of {content_dir}",
        "To be able to publish any files you have to create a .boink file "
...
def _read_config(content_dir: DirName, config_path: Path) -> tuple[Config, Path | None]:
    """### Read configuration from a path (file or dir)

    Returns configuration dictionary and optional config asset path.
    """

    # if os.path.isfile(config_path):
    #     config_file = os.path.join(base_dir, config_path)
    #     config_asset_path = None
    # else:
    #     # seek config file

    #     config_file = os.path.join(base_dir, config_path, seeked_file)
    #     config_asset_path = os.path.join(base_dir, config_path)
